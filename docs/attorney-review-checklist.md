...
### M-7 · Jurisdiction Procedure Deadlines and State-Specific Timeline Guidance

**Risk:** Medium. `shared/jurisdiction-procedure-rules.ts` is the single source of truth for arraignment, bail-hearing, speedy-trial, preliminary-hearing, and discovery-deadline facts across all 50 states + DC + federal + five territories. It feeds directly into the AI guidance prompt (`buildJurisdictionContextBlock`) as authoritative or qualified fact, and into the legal accuracy validator. A wrong deadline here doesn't just sit in an unused data file; it can come back to a user inside AI-generated guidance as though it were verified.
...
**Content location:**
- `shared/jurisdiction-procedure-rules.ts`: full file; header comment (lines 1–46) documents verification status, the five territory records, and fields whose deadline text remains an estimate. The individually reviewed jurisdictions carry dated, sourced inline comments above `lastVerified`.
- `server/services/claude-guidance.ts` — `buildJurisdictionContextBlock`, where this data enters the AI prompt
- `client/src/components/legal/guidance-dashboard.tsx`: timeline, deadline cards, and the warning shown when a timeframe is an estimate
- `client/src/components/legal/guidance-print-plan.tsx`: timeline and deadline content used by print/PDF output
- `docs/evals-coverage.md`: state coverage matrix and batch-2 discovery verification notes

The review should explicitly include the newly added state deadline entries and batch-2 discovery characterizations, all five territory procedure records, and South Carolina's preliminary-hearing demand language. The timeline and estimate-warning behavior is tracked as an engineering release check; the attorney's role is to confirm that the underlying legal characterization and qualification are accurate.

**Legal question for attorney:**
For a sample of jurisdictions **not** in the 9 already re-verified this cycle (suggested: 3–5 states your team considers high-traffic or high-risk), are the arraignment, speedy-trial, preliminary-hearing, and discovery-deadline figures in `jurisdiction-procedure-rules.ts` still accurate against current statute/rule text? Are the newly added state entries and batch-2 discovery characterizations legally sound? Are the territory rules appropriately qualified and clearly distinguished from state rules? Is South Carolina's preliminary-hearing demand language accurate, including when a demand is required and what deadline applies? Given that a prior "mass date bump" was found to have updated timestamps without actual verification, should the platform's maintenance process require documented sourcing (statute citation + date checked) for every future `lastVerified` change, the way the individually reviewed entries now do?
