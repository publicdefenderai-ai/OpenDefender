export default {
  ...
  "case": {
    ...
    "clearSession": {
      "title": "清除会话数据",
      "message": "这将永久删除内存中您所有的案件信息和指导内容。此操作无法撤销。",
      "confirm": "清除会话",
      "cancel": "取消",
      "successTitle": "会话已清除",
  ...
  "privacyPolicy": {
    ...
    "principles": {
      ...
      "noPersonalData": {
        "title": "不收集个人数据",
        "description": "我们不收集姓名、联系信息、案件编号或任何能识别您身份的其他信息。当您使用案件路线图或AI聊天工具时，您提供的输入——司法管辖区、指控类型、案件阶段和关注点——仅保存在临时服务器内存中，并在24小时内自动删除。这些信息不会写入持久数据库，也不会与任何身份关联。"
      },
      "anonymizedData": {
        "title": "仅使用匿名数据",
    ...
    "caseData": {
      ...
      "memoryOnly": {
        "title": "存储什么，不存储什么",
        "description": "您的输入仅保存在服务器内存中：司法管辖区、指控类型、案件阶段、羁押状态、已选关注点以及生成的AI指导。从不收集的信息：您的姓名、地址、电话号码、电子邮件、案件编号或任何身份识别信息。您提供的数据是分类信息（一个州、一种指控类型、是或否），而不是自由文本形式的案件事实。"
      },
      "piiRedaction": {
        "title": "额外保障：AI处理前移除个人信息",
        "description": "在数据到达AI之前，我们会自动扫描并移除电话号码、电子邮件地址和社会安全号码等个人信息。即使您在自由文本字段中输入了身份识别信息，这一保障也适用。您的个人信息不会发送到外部AI服务。"
      },
      "autoDelete": {
        "title": "24小时后自动删除",
        "description": "内存中的会话数据在24小时后自动过期并永久删除。无需任何操作。每次服务器重启也会清除所有会话数据。"
      },
      "serverRestart": {
        "title": "完全不存储数据的工具",
    ...
    "technical": {
      ...
      "sessions": {
        "title": "会话数据与 Cookie",
        "description": "OpenDefender 不对公共访客使用追踪 Cookie 或持久性标识符。只有当律师登录律师门户时才会使用会话 Cookie——它在律师访问期间保持身份验证，并在律师退出登录或关闭浏览器时删除。浏览网站、使用 AI 指导或使用律师工具的公共访客不会收到任何 Cookie。不会永久存储任何会话信息。"
      },
      "logs": {
        "title": "服务器日志",
      ...
      "external": {
        ...
        "services": {
          "anthropic": "Anthropic Claude Sonnet 4.6 - 用于案件路线图、引导式案件支持工具、文件摘要工具、信函生成工具、律师文件生成工具，以及减刑备忘录生成器中可选的“AI 润色”步骤。个人信息在处理前会被移除。Anthropic 可能会出于运营和安全目的保留数据最多30天，之后自动删除。这些数据不会用于训练 AI 模型。",
  ...
  "attorneyPortal": {
    ...
    "verify": {
      ...
      "privacyNotice": "您的会话数据将在1小时后自动删除。",
      "verifying": "正在验证……",
      "submit": "验证并继续"
    },
    "session": {
  ...
  "bailPrep": {
    ...
    "printChecklist": {
      ...
      "button": "打印清单",
      "disclaimer": "此清单仅为实用工具。不构成法律建议，不受特权保护。"
    }
  },
  "disclaimers": {
    "hero": {
    ...
    "aiDisclosure": {
      "title": "人工智能技术披露",
      "p1": "OpenDefender 上有五项功能使用 AI 生成内容：案件路线图和引导式案件支持工具、律师门户中的律师文件生成工具、文件摘要工具、信函生成工具，以及减刑备忘录生成器中可选的“AI 润色”步骤。这五项功能均使用 Anthropic 的 Claude Sonnet 4.6。",
      "p2": "案件路线图还包含一个基于规则的备用引擎，当 AI 服务不可用时会自动启动。在备用模式下，指导内容由结构化法律规则生成，而非实时 AI 模型——在这种情况下不会向 Anthropic 发送任何数据。律师文件生成和文件摘要工具没有此备用功能，如果 AI 服务不可用，将显示错误。",
      "p3": "本网站上的所有其他内容——权利信息、犯罪记录清除资格筛查工具、法庭日期指南、移民指南、支持资源以及所有其他静态页面——均不使用 AI，均经过人工研究和编写。",
      "p4": "案件路线图还包含一个民事紧急分诊步骤，就当前情况（住房、就业、受抚养人、移民）提出分类问题。这些回答与所有其他案件路线图输入遵循相同的仅限会话路径：保存在服务器内存中，24小时后自动删除，从不写入数据库。",
      "p5": "AI 生成的指导不能替代持牌律师的建议。我们通过法律数据库交叉验证回答以提高准确性，但 AI 可能出错。在做出法律决定之前，请咨询有资质的律师。",
      "p6_pre": "在您的输入到达 AI 之前，我们会自动扫描并删除个人信息（姓名、电话号码、地址、社会安全号码）。有关数据处理的完整详情，请参阅我们的",
      "p6_post": "。"
    },
    "screeningTools": {
      "title": "筛查与资格评估工具",
      ...
      "p4": "减刑备忘录生成器帮助律师和倡导者将客户信息整理成格式化的量刑备忘录。核心工具完全在您的浏览器中运行。备忘录在本地生成为 .docx 文件，不向我们的服务器传输或存储任何数据。另有一项独立的可选功能“AI 润色”，可将您输入的结构化内容转换为叙述性文字。如果您使用该功能，您输入的字段值将发送到我们的服务器并转发给 Claude 进行一次性处理。之后不会记录、缓存或存储任何内容。",
