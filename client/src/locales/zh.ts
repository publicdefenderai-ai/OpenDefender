export default {
  ...
  "privacyPolicy": {
    ...
    "caseData": {
      ...
      "serverRestart": {
        "title": "完全不存储数据的工具",
        "description": "以下几种工具完全不发出任何服务器请求：记录清除资格评估工具、附带后果筛查工具、公设辩护人入档清单和减轻情节备忘录生成器。这四种工具均完全在您的浏览器中运行。两种律师工具在您的设备本地生成可下载的 .docx 文件——不会向我们的服务器传输任何数据。网站上所有其他静态页面（权利指南、支持资源、法庭日期指南、移民指南）同样不收集数据。"
      }
    },
    "noDataTools": {
      "title": "不收集任何数据的工具",
      "description": "记录清除资格评估工具、附带后果筛查工具、公设辩护人入档清单和减轻情节备忘录生成器在任何时候都不发出服务器请求。评估工具完全在您的浏览器中作为决策树运行。律师工具在您的设备本地生成可下载的 .docx 文件。使用这些工具时，不会以电子方式提交、传输或存储任何信息。"
    },
    "technical": {
      "title": "技术详情",
      "sessions": {
      ...
      "external": {
        ...
        "services": {
          "anthropic": "Anthropic Claude Sonnet 4.6 - 用于案件路线图、文件摘要工具和AI聊天。个人信息在处理前会被移除。Anthropic 可能会出于运营和安全目的保留数据最多30天，之后自动删除。这些数据不会用于训练 AI 模型。",
  ...
  "disclaimers": {
    ...
    "aiDisclosure": {
      "title": "人工智能技术披露",
      "p1": "OpenDefender 上有三项功能使用 AI 生成内容：案件路线图和引导式案件支持工具、律师门户中的律师文件生成工具，以及文件摘要工具。这三项功能均使用 Anthropic 的 Claude Sonnet 4.6。",
    ...
    "screeningTools": {
      ...
      "p4": "减刑备忘录生成器帮助律师和倡导者将客户信息整理成格式化的量刑备忘录。不使用 AI，也不进行服务器调用。备忘录完全在您的浏览器中生成为 .docx 文件——不向我们的服务器传输或存储任何数据。",
