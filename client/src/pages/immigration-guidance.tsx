...
const RAPID_REGIONS: RapidRegion[] = [
  {
    id: 'sfbay',
...
function HotlineCard({ entry, t, prominent = false }: { entry: HotlineEntry; t: (key: string) => string; prominent?: boolean }) {
  return (
    <div className={`rounded-lg border p-4 bg-white dark:bg-gray-900/60 flex flex-col gap-2 ${prominent ? 'border-red-400 dark:border-red-600 shadow-sm' : 'border-red-200 dark:border-red-900/60'}`}>
      <p className="text-xs font-semibold text-muted-foreground leading-tight">{entry.org}</p>

      {entry.isText ? (
        <div className="flex items-baseline gap-1.5">
          <span className="text-xs text-red-700 dark:text-red-400 font-medium">{t('immigration.rapidResponse.textTo')}</span>
          <a
            href={`sms:${entry.tel}`}
      ...
      {entry.note && (
        <p className="text-xs text-muted-foreground italic leading-snug">{entry.note}</p>
      )}
...
function RapidResponseSection() {
  ...
  return (
    <section id="rapid-response" className="py-10 bg-red-50 dark:bg-red-950/20 border-y border-red-200 dark:border-red-900/40">
      <div className="max-w-4xl mx-auto px-4">
        ...
        {localHotlines.length > 0 && (
          <div className="mb-6">
            ...
            <div className="grid sm:grid-cols-2 gap-3 mb-5">
              {localHotlines.map(h => <HotlineCard key={h.id} entry={h} t={t} prominent />)}
            </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {NATIONAL_HOTLINES.map(h => <HotlineCard key={h.id} entry={h} t={t} />)}
        </div>
